(function($) {
  ...
  _.extend(Backbone.View.prototype, {
    ...
    stickit: function(optionalModel, optionalBindingsConfig) {
      var self = this,
        observeModelEvent, model = optionalModel || this.model,
      ...
      _.each(_.keys(bindings), function(selector) {
        var getVal, modelEvents, eventCallback, format, modelAttr, attributes, config = bindings[selector] || {},
          bindKey = _.uniqueId(),
          $el = self.$(selector);
        ...
        if (modelAttr) {
          ...
          if ($el.is('input[type=radio]') || $el.is('input[type=checkbox]') || $el.is('select')) self.events['change ' + selector] = eventCallback;
          ...
          observeModelEvent('bind:' + modelAttr, function(val, options) {
            if (options && options.bindKey != bindKey) updateViewBindEl(self, $el, config, getVal(modelAttr), model);
          });
  ...
  Backbone.Model.prototype.set = _.wrap(Backbone.Model.prototype.set, function(oldSet, key, value, oldOptions) {
    var attrs, attr, val, ret, options = oldOptions && _.clone(oldOptions) || {},
      now = this.attributes;
    ...
    _.each(_.keys(attrs || {}), _.bind(function(attr) {
      ...
      if (!_.isEqual(now[attr], val) || (options.unset && _.has(now, attr))) this.trigger('bind:' + attr, attrs[attr], options);
    }, this));
  ...
  evaluatePath = function(obj, path) {
    ...
    return _.reduce(pathParts, function(memo, i) {
      return memo[i];
    }, obj) || obj;
  };
  ...
  updateViewBindEl = function(view, $el, config, val, model, isInitializing) {
    var markReadonly, originalVal, tempSelection, modelAttr = config.modelAttr,
    ...
    if ($el.is('input[type=radio]')) {
      ...
      var optList, list = selectConfig.collection,
        fieldVal = model.get(modelAttr);

      // Get the current selected option value if the select options exist.
      if ($el[0].options.length && $el[0].selectedIndex >= 0) originalVal = $($el[0].options[$el[0].selectedIndex]).data('stickit_bind_val');
      ...
      if (fieldVal == null) $el.append('<option/>').find('option').prop('selected', true).data('stickit_bind_val', fieldVal);
      ...
      _.each(optList, function(obj) {
        ...
        if (optionVal != null && fieldVal != null && optionVal == fieldVal || (_.isObject(fieldVal) && _.isEqual(optionVal, fieldVal))) option.prop('selected', true);
